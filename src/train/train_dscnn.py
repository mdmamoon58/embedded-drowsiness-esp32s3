#!/usr/bin/env python3
"""
Train a lightweight depthwise-separable CNN (DS-CNN) for TinyML deployment.
Uses the same labels_subjects.csv as the baseline model.
"""
...
BATCH_SIZE = 32
EPOCHS = 40
MODEL_OUT = Path("models/dscnn_tiny.h5")
...
def build_dscnn(num_classes: int) -> tf.keras.Model:
    inputs = tf.keras.layers.Input(shape=(IMG_SIZE[1], IMG_SIZE[0], 1))

    x = conv_block_ds(inputs, 16)               # 96x96x16
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)   # 48x48x16

    x = conv_block_ds(x, 32)                    # 48x48x32
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)   # 24x24x32

    x = conv_block_ds(x, 64)                    # 24x24x64
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)   # 12x12x64

    # Global average pooling to keep params small
    x = tf.keras.layers.GlobalAveragePooling2D()(x)         # 64

    x = tf.keras.layers.Dense(32, activation="relu")(x)
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.Model(inputs=inputs, outputs=outputs, name="dscnn_tiny")
...
def main():
    ...
    model = build_dscnn(num_classes=len(class_names))
    ...
    print(f"[INFO] Saved DS-CNN model to {MODEL_OUT}")
